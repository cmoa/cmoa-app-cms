class ApiV2Controller < ApplicationController
  ...
  def check_params
    ...
    if ((token.nil? || token.empty?) ||
      ...
      return render :json => {:status => false, :message => 'Invalid login', :code => 1}
    end

    # Verify the token
    return render :json => {:status => false, :message => 'Invalid login', :code => 2} if token != API_TOKEN
    ...
    return render :json => {:status => false, :message => "Invalid Login", :code => 3, :hash => hashable} if signature1 != signature2
  end
